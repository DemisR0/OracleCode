WITH raw_data AS (
SELECT  sysst.dbid, sysst.snap_id, sysst.instance_number, begin_interval_time ,end_interval_time ,  startup_time,
...
sysst.VALUE - lag (sysst.VALUE) OVER ( PARTITION BY  sysst.dbid, sysst.instance_number, startup_time
                ORDER BY sysst.dbid, sysst.instance_number, startup_time, begin_interval_time) as stat_value,
...
WHERE stt_name.stat_name='redo size'
...
SELECT rd.wday as day , cast(rd.dhour as int) as hour ,round(SUM(rd.stat_value/rd.delta_t),0) as redo_bytes_sec
