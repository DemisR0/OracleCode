...
sysst.VALUE - lag (sysst.VALUE) OVER ( PARTITION BY  sysst.dbid, sysst.instance_number, startup_time,stt_name.stat_name
...
WHERE stt_name.stat_name in ('physical read total bytes','physical write total bytes')
...
SELECT rd.wday as day , cast(rd.dhour as int) as hour ,round(SUM(rd.stat_value/rd.delta_t)),0) as IO_Bytes_sec
