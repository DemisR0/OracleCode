...
-- laisser les ^M necessaires pour screen
        ...
        col begin_interval_time format a24
        col end_interval_time format a24
        ...
        with io_by_func as (
        select  snaps.dbid,sysst.snap_id, sysst.instance_number, begin_interval_time ,end_interval_time ,  startup_time, filetype_name,
        SMALL_READ_MEGABYTES - lag (SMALL_READ_MEGABYTES) over ( partition by  snaps.dbid,startup_time, sysst.instance_number, filetype_name 
                        order by sysst.snap_id,begin_interval_time) small_reads_mb,
        SMALL_WRITE_MEGABYTES - lag (SMALL_WRITE_MEGABYTES) over ( partition by  snaps.dbid,startup_time, sysst.instance_number, filetype_name 
                        order by sysst.snap_id,begin_interval_time) small_writes_mb,
        LARGE_READ_MEGABYTES - lag (LARGE_READ_MEGABYTES) over ( partition by  snaps.dbid,startup_time, sysst.instance_number, filetype_name 
                        order by sysst.snap_id,begin_interval_time) large_read_mb,
        LARGE_WRITE_MEGABYTES - lag (LARGE_WRITE_MEGABYTES) over ( partition by  snaps.dbid,startup_time, sysst.instance_number, filetype_name 
                        order by sysst.snap_id,begin_interval_time) large_write_mb,
        SMALL_READ_REQS - lag (SMALL_READ_REQS) over ( partition by  snaps.dbid,startup_time, sysst.instance_number, filetype_name 
                        order by sysst.snap_id,begin_interval_time) small_read_reqs,
        SMALL_WRITE_REQS - lag (SMALL_WRITE_REQS) over ( partition by  snaps.dbid,startup_time, sysst.instance_number, filetype_name 
                        order by sysst.snap_id,begin_interval_time) small_write_reqs,
        LARGE_READ_REQS - lag (LARGE_READ_REQS) over ( partition by  snaps.dbid,startup_time, sysst.instance_number, filetype_name 
                        order by sysst.snap_id,begin_interval_time) large_read_reqs,
        LARGE_WRITE_REQS - lag (LARGE_WRITE_REQS) over ( partition by  snaps.dbid,startup_time, sysst.instance_number, filetype_name 
        ...
        from DBA_HIST_IOSTAT_FILETYPE sysst , DBA_HIST_SNAPSHOT snaps
        where snaps.snap_id = sysst.snap_id
        and snaps.dbid =sysst.dbid
        and sysst.instance_number=snaps.instance_number
        and begin_interval_time > sysdate-1 -- Nb of days
        )
        select '|'||di.host_name||','||di.instance_number||','||io.snap_id||','||di.instance_name||','||to_char(io.end_interval_time,'YYYYMMDD HH24:MI')||','||io.filetype_name||','||
