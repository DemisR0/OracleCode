...
def getXterm(sName,display = ':1'):
    print('export DISPLAY=' + display + '; nohup xterm -fg white -bg black -title ' + sName + '-sb -geometry 150x30+100+100 -fa lucidasans-10 -sl 500000 -e /bin/bash -c "tmux new -s ' + sName + ' -n ' + sName + '" &')
    if os.system('export DISPLAY=' + display + '; nohup xterm -fg white -bg black -title ' + sName + '-sb -geometry 150x30+100+100 -fa lucidasans-10 -sl 500000 -e /bin/bash -c "tmux new -s ' + sName + ' -n ' + sName + '" &') == 0:
        os.system('tmux send-key -t ' + sName + ' \'tmux set-option history-limit 5000\' Enter')
        return True
    else:
        return False
