...
mkdir -p /u02/app/oracle/oradata/${ORACLE_SID}/${ORACLE_SID}_AMS/controlfile
mkdir -p /u03/app/oracle/fast_recovery_area/${ORACLE_SID}/${ORACLE_SID}_AMS/controlfile
mkdir -p /u04/app/oracle/redo/${ORACLE_SID}/${ORACLE_SID}_AMS/controlfile/
mkdir -p /u03/app/oracle/redo/${ORACLE_SID}/${ORACLE_SID}_AMS
mkdir -p /u04/app/oracle/redo/${ORACLE_SID}/${ORACLE_SID}_AMS
mkdir -p /u01/app/oracle/admin/${ORACLE_SID}/adump
mkdir -p /u02/app/oracle/broker/${ORACLE_SID}
mkdir -p /u02/app/oracle/oradata/${ORACLE_SID}
mkdir -p /u01/app/oracle/wallets/tde/$ORACLE_SID
printf 'done\n***********\n' >> $LOG

printf '***********\ntrying to start the database and restore the control file : \n' >> $LOG
printf "STARTUP NOMOUNT;
RESTORE STANDBY CONTROLFILE FROM '${CTLFILEBCK}';
...
printf "***********\n restore database : \n"
rman target / cmdfile='restore'${ORACLE_SID}'_database.rman' > restore${ORACLE_SID}_database.log
...
printf "then clear logfile groups \n"
printf "then add tempfiles if needed after cutover \n"
